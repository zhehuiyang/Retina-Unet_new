...
# 训练集太少，采用分块的方法进行训练
def get_data_training(DRIVE_train_imgs_original, #训练图像路径
    ...
    train_imgs = train_imgs[:,:,9:574,:]  #cut bottom and top so now it is 565*565  # 图像裁剪 size=565*565
    train_masks = train_masks[:,:,9:574,:]  #cut bottom and top so now it is 565*565 # 图像裁剪 size=565*565
    data_consistency_check(train_imgs,train_masks) # 训练图像和金标准图像一致性检查

    #check masks are within 0-1
    assert(np.min(train_masks)==0 and np.max(train_masks)==1) #金标准图像 2类 0-1
    ...
    data_consistency_check(patches_imgs_train, patches_masks_train)  # 训练图像子块和金标准图像子块一致性检查
...
def get_data_testing_overlap(DRIVE_test_imgs_original, DRIVE_test_groudTruth, Imgs_to_test, patch_height, patch_width, stride_height, stride_width):
    ...
    test_imgs = test_imgs[0:Imgs_to_test,:,:,:]
    test_masks = test_masks[0:Imgs_to_test,:,:,:]
    test_imgs = paint_border_overlap(test_imgs, patch_height, patch_width, stride_height, stride_width) # 拓展图像 可以准确划分
    ...
    #extract the TEST patches from the full images
    # 按照顺序提取图像快 方便后续进行图像恢复（作者采用了overlap策略）
...
def extract_random(full_imgs,full_masks, patch_h,patch_w, N_patches, inside=True):
    if (N_patches%full_imgs.shape[0] != 0): # 检验每张图像应该提取多少块
        ...
        exit()
    assert (len(full_imgs.shape)==4 and len(full_masks.shape)==4)  #4D arrays   # 张量尺寸检验
    assert (full_imgs.shape[1]==1 or full_imgs.shape[1]==3)  #check the channel is 1 or 3  # 通道检验
    assert (full_masks.shape[1]==1)   #masks only black and white  # 通道检验
    ...
    patch_per_img = int(N_patches/full_imgs.shape[0])  #N_patches equally divided in the full images # 每张图像中提取的子块数量
    print("patches per full image: " +str(patch_per_img))
    iter_tot = 0   #iter over the total numbe rof patches (N_patches)  # 图像子块总量计数器
    for i in range(full_imgs.shape[0]):  #loop over the full images    # 遍历每一张图像
        k=0  # 每张图像子块计数器
        while k <patch_per_img:
            ...
            if inside==True:
                if is_patch_inside_FOV(x_center,y_center,img_w,img_h,patch_h)==False:
                    continue
            patch = full_imgs[i,:,y_center-int(patch_h/2):y_center+int(patch_h/2),x_center-int(patch_w/2):x_center+int(patch_w/2)]    # 块中心的范围
            ...
            iter_tot +=1   #total  # 子块总量计数器
            k+=1  #per full_img    # 每张图像子块总量计数器
...
def extract_ordered_overlap(full_imgs, patch_h, patch_w,stride_h,stride_w):
    ...
    N_patches_img = ((img_h-patch_h)//stride_h+1)*((img_w-patch_w)//stride_w+1)  #// --> division between integers  # 每张图像采集到的子图像
    N_patches_tot = N_patches_img*full_imgs.shape[0]  # 测试集总共的子图像数量
...
def recompone_overlap(preds, img_h, img_w, stride_h, stride_w):
    assert (len(preds.shape)==4)  #4D arrays # 检查张量尺寸
    ...
    k = 0 #iterator over all the patches  #迭代所有的子块
